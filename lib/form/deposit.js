...
export default class DepositForm {
    ...
    async getFeeAllToken() {
        ...
        for (const token of Object.keys(window._tpayment.configs.AssetByTokenName)) {
            ...
            if (this._isUseToko) {
                ...
                const feeDiscountBig = ethers.parseUnits(feeDiscount.toString());
                this._serviceFee[token]['feeDiscount'] = feeDiscount;
                this._serviceFee[token]['totalFee'] = feeBig - feeDiscountBig;
            }
        }
    ...
    async tokenApprove(tokenAddress, contractAddress) {
        ...
        return await smc
            .connect(signer)
            .approve(contractAddress, ethers.MaxUint256, { gasLimit: window._tpayment.configs.GasLimit });
    }
