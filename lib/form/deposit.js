...
export default class DepositForm {
    ...
    async onDeposit() {
        ...
        const fee = ethers.utils.parseEther((this._serviceFee - this._serviceFeeDiscount).toString()).toBigInt();
    ...
    async show() {
        ...
        if (this._isUseToko) {
            const tokoAllowance = await this.hasEnoughAllowance(Configs.AssetByTokenName.TOKO, tokoAmount, Configs.TchainContractAddress);
            if (!tokoAllowance){
        ...
        console.log("DONE ===>getTransactionFee");
        ...
        const tabs = document.getElementById("_tchain-tabs");
        tabs.querySelectorAll('.item').forEach(item => {
            item.addEventListener('click', () => this.onChangeTab(item));
        })
    }
}
