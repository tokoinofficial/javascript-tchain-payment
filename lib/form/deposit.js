...
export default class DepositForm {
    constructor(params={}) {
        this._currency = params.currency || Configs.DefaultCurrency;
        this._notes = params.notes;
        this._chainId = params.chain_id;
        this._tokenAddress = Configs.AssetByTokenName[Configs.DefaultToken];
        this._tokenName = Configs.DefaultToken;
    ...
    async getRates() {
        try {
            ...
            headers.append("public-api-key", window._tpayment.apiKey);
            ...
            const res = await fetch(`${Configs.TChainAPIURL}/t-chain-sdk/exchange-rate`, requestOptions);
            const dataJSON = await res.json();
            this._rates = {...Rates, ...dataJSON.result.data};
        }
        catch(err) {
    ...
    validateCurrency(currency) {
        if (!Configs.CurrencySupport.includes(currency)) {
