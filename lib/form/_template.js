...
function generateOptionPayFee(){
    let html = `<div class="discountFeeOptions">
        <div class="desc">
            To get 10% discount of the service fee, just pay a small amount with:
        </div>
    `;
    const assetPayFees  = Configs.AssetPayFees;
    const icons = Configs.Icons;
    assetPayFees.forEach(asset => {
        html += `
        <div class="_option">
            <div class="info">
                <div class="check">
                    <input type="radio" id="_tchain.deposit.cbx.pay.with.${asset}" name="assetPayFee" value="${asset}" />
                    <label for="_tchain.deposit.cbx.pay.with.${asset}"></label>
                </div>
                <div class="icon">${icons[asset]}</div>
                <div class="discountFee">1000</div>
            </div>
            <div class="balance">
                <span style="font-size: 11px; color: #3C3C43; font-weight: 400">Balance</span>
                <div style="display: flex; align-items: center">
                    <div class="icon">${icons[asset]}</div>
                    <span class="amount">1,000.0</span>
                </div>
            </div>
        </div>
        `;
...
function generateAssetOptions(params={}) {
    const optionPayFees = generateOptionPayFee();
    let html = "";
    const assets = params.Assets;
    const selected = params.tokenName;
    const icons = Configs.Icons;
    const assetPayFees  = Configs.AssetPayFees;
    for (const asset of Object.keys(assets)) {
        const assetInfo = assets[asset];
        html += `
        <div class="_tchainPaymentToken ${asset === selected ? 'active' : ''}">
            <div class="_tchainPaymentTokenHeader">
                <div class="_tokenInfo">
                    <div class="_tokenIcon">${icons[asset]}</div>
                    <div class="_tokenName">
                        ${assetInfo.Name}<br />
                        <label>${assetInfo.Type}</label>
                    </div>
                </div>
                <div class="_tokenAction">
                    <input type="radio" id="_tchain.payment.token.${asset}" name="_radioTchainPaymentToken" value="${asset}" ${
                        selected === asset ? 'checked' : ''
                    } />
                    <label for="_tchain.payment.token.${asset}"></label>
                </div>
            </div>
            <div class="_tchainPaymentTokenContent">
                ...
                <div class="_item">
                    <div>Service Fee:</div>
                    <div><span class="nfm">${assetInfo.fee}</span> ${asset}</div>
                </div>
                <div class="_item">
                    <div>Exchange Rate:</div>
                    <div>
                        1 ${asset} =~ 
                        <span class='nfm'>${assetInfo.rate}</span>
                        ${params.currency}
                    </div>
                </div>
                ${
                    selected === asset && !assetPayFees.includes(asset) ? 
                    `
