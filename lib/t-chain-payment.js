async function init(params = {}) {
    const { default: Configs, TestConfigs } = await import(/* webpackPreload: true */ './config');
...
async function loadWeb3() {
    ...
    if (ethereum !== undefined) {
        ...
        const { ethers } = await import(/* webpackPreload: true */ 'ethers');
...
async function deposit(req, callback) {
    const [{ DepositReq }, { default: DepositForm }] = await Promise.all([
        import(/* webpackPreload: true */ './serializers'),
        import(/* webpackPreload: true */ './form/deposit'),
    ]);
...
async function generateQrCode(req = {}) {
    const [{ DepositReq }, QRCode] = await Promise.all([
        import(/* webpackPreload: true */ './serializers'),
        import(/* webpackPreload: true */ 'qrcode'),
    ]);
